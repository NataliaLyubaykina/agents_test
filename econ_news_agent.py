...
Usage (in a notebook):
----------------------
!pip install feedparser pandas scikit-learn python-dateutil pytz openai
from econ_news_agent import EconNewsAgent
agent = EconNewsAgent()
res = agent.build("2025-09-11")
...
CLI:
----
python econ_news_agent.py --date 2025-09-11
"""
...
def same_calendar_day(dt_utc: Optional[datetime], target_date_local: datetime) -> bool:
...
def entries_to_dataframe(entries: list, target_date_local: datetime) -> pd.DataFrame:
    """Filter entries to the given local date and convert to a DataFrame."""
    filtered = [e for e in entries if same_calendar_day(e["published_dt_utc"], target_date_local)]
...
def gpt_daily_summary(df: pd.DataFrame, date_local: datetime) -> str:
...
class EconNewsAgent:
    def __init__(self, feeds=None, local_tz=LOCAL_TZ, model=OPENAI_MODEL, max_k=6, use_rss_threshold=0.4):
        self.feeds = feeds or YAHOO_FINANCE_FEEDS
    ...
    def build(self, date_str: Optional[str] = None) -> DigestResult:
        ...
        df = entries_to_dataframe(entries, target_date_local=target_date_local)
        total = len(df)
